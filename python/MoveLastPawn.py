        ...
        while not Player.stop_move_generation(self, rc):
            ...
            for pawn in sorted_pawns:
                ...
                if isinstance(pawn_space, StartSpace):
                    print("making an EnterPiece in doMove")
                    new_move = EnterPiece(pawn)
                    valid, rc = Player.check_next_move(self, rc, new_move)
                    if valid:
                        print("EnterPiece was valid using pawn "+str(pawn.id))
                        moves.append(new_move)
                        print("length of moves: "+str(len(moves)))
                    else:
                        new_move = None
                else:  #Regular Space
                    print("making a MoveMain in doMove")
                    for die in rc.tvals.get_all_dice():
                        ...
                        if valid:
                            ...
                            new_move = None
                if new_move != None:
                    print("breaking from pawn loop")
                    print("rc final green pawn 3 location: "+str(rc.b_final.pawns["green"][3].location))
                    break
        return moves
